...
import { db } from '.';
import { admins, categories, products, productImages, settings } from './tables';
...
export async function seedDatabase(): Promise<void> {
  ...
  for (const prod of initialProducts) {
    const [catRow] = await db.select({ name: categories.name })
      .from(categories)
  ...
  await db.insert(settings).values({
    ...
    secondary_bank_name: initialSettings.secondary_bank_name || '',
    secondary_account_number: initialSettings.secondary_account_number || '',
    secondary_account_holder: initialSettings.secondary_account_holder || '',
